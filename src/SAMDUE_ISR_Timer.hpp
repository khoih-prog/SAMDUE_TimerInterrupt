...
#define SAMDUE_ISR_Timer SAMDUE_ISRTimer

typedef void (*timerCallback)();
typedef void (*timerCallback_p)(void *);

class SAMDUE_ISR_Timer 
{
...
#define TIMER_DEFCALL_RUNANDDEL 2       // call the callback function and delete the timer
    ...
    // returns the timer number (numTimer) on success or
    // -1 on failure (f == NULL) or no free timers
    int setupTimer(const unsigned long& d, void* f, void* p, const bool h, const unsigned& n);
